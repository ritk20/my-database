...
for cmd, expect in zip(cmds, outputs):
    ...
    if out != expect:
        ...
        # Add debugging output to print the state of the sorted set
        if 'zadd' in cmd:
            print('State of sorted set after zadd:')
            state = subprocess.check_output(shlex.split('./client zquery zset 1 "" 0 10')).decode('utf-8')
            print(state)
