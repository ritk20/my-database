...
static void out_err(std::string &out, int32_t code, const std::string &msg)
{
    ...
    std::uint32_t len = (std::uint32_t)msg.size();
    out.append((char *)&len, 4);
    out.append(msg);
}

static void out_arr(std::string &out, std::uint32_t n)
{
    out.push_back(SER_ARR);
    out.append((char *)&n, 4);
}

static size_t begin_arr(std::string &out)
{
    out.push_back(SER_ARR);
...
static void end_arr(std::string &out, size_t ctx, std::uint32_t n)
{
    size_t pos = ctx;
